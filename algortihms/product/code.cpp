...
void writeObstaclesCSV(const string& name, const vector<Obstacle>& obs, double inflate){
    ofstream f(name);
    f << "cx,cy,half\n";
    for(auto& o: obs) f << o.cx << "," << o.cy << "," << (o.half + inflate) << "\n";
}
...
int main()
{
    ...
    cout << "Lazy Theta*:  collisions=" << Ml.collisions
         << "  length=" << fixed << setprecision(3) << Ml.length
         << " m  smooth=" << Ml.smooth << " rad\n";
    cout << "B-spline:     collisions=" << Mb.collisions
         << "  length=" << fixed << setprecision(3) << Mb.length
         << " m  smooth=" << Mb.smooth << " rad\n";

    cout << "\n=== B-spline improvement vs Lazy Theta* ===\n";
    cout << "Length gain:     " << fixed << setprecision(2) << pct_gain(Ml.length, Mb.length) << " %\n";
    cout << "Smoothness gain: " << fixed << setprecision(2) << pct_gain(Ml.smooth, Mb.smooth) << " %\n";
    cout << "Collision delta: " << (Ml.collisions - Mb.collisions) << " (positive means fewer collisions)\n";
    ...
    system("python3 ./plot.py obstacles.csv lazy.csv bspline.csv meta.json");
