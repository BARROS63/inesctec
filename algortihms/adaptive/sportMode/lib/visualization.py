"""
Visualization module: Common plotting utilities for all spline types and comparisons.
"""
...
def plot_raw_path(width: int, height: int, obstacles: Set[Tuple[int, int]], 
                   path: List[Tuple[float, float]], fname: str):
    """Plot raw grid, obstacles, and path."""
...
def plot_spline_comparison(width: int, height: int, obstacles: Set[Tuple[int, int]], 
                           path: List[Tuple[float, float]], 
                           spline_dict: Dict[str, List[Tuple[float, float]]],
                           length_dict: Dict[str, float],
                           fname: str):
    """Plot all splines overlaid with path length annotations.
    
    Args:
        width: Grid width.
        height: Grid height.
        obstacles: Set of obstacle coordinates.
        path: Raw path waypoints.
        spline_dict: Dict mapping spline name -> sampled points.
        length_dict: Dict mapping spline name -> path length.
    ...
    ax.set_ylim(-0.5, height + 0.5)
    ax.set_aspect('equal')
    ax.set_title('Spline Comparison (sport mode = shortest)')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.legend(loc='best')
    plt.grid(False)
...
def print_path_statistics(name: str, length: float, num_points: int):
    """Print formatted path statistics."""
