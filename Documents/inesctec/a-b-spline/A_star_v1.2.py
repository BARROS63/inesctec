...
def main():
    ...
    grid_height = max(start_grid[1], goal_grid[1]) + 20
    ...
    # Inflate obstacle according to drone radius
    drone_radius_m = 0.5375  # Radius (1,075 m diameter)
    resolution = 0.5  # cells size
    inflation_radius = int(np.ceil(drone_radius_m / resolution))  # margin cells number
    ...
    for dx in range(-inflation_radius, inflation_radius):
        for dy in range(-inflation_radius, inflation_radius):
            ox = mid_x + dx
            oy = mid_y + dy
            if 0 <= ox < grid.shape[1] and 0 <= oy < grid.shape[0]:
                grid[ox, oy] = 1  # phorbiden zone
