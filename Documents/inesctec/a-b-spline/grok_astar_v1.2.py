...
def visualize_path(grid: np.ndarray, path: List[Tuple[int, int]], start: Tuple[int, int], goal: Tuple[int, int], obstacle: Tuple[int, int]):
    ...
    plt.scatter(obstacle[1], obstacle[0], c='black', s=100, label='Obstacle', edgecolors='black')
    
    plt.xticks(np.arange(0, grid.shape[1], 1))
    plt.yticks(np.arange(0, grid.shape[0], 1))
    plt.grid(True, which='both', color='black', linewidth=0.5)
    plt.legend()
    plt.title("A* Pathfinding with Random Obstacle")
    plt.show()
...
def main():
    ...
    # Define the 4x8 meter obstacle placement area in grid coordinates
    mid_x = (start_grid[0] + goal_grid[0]) // 2
    mid_y = (start_grid[1] + goal_grid[1]) // 2
    obstacle_area_x = range(mid_x - 2, mid_x + 3)  # 4 meters wide (2 cells each side)
    obstacle_area_y = range(mid_y - 4, mid_y + 4)  # 8 meters tall (4 cells each side)
    
    # Main loop for dynamic obstacle placement
    try:
        while True:
            # Create fresh grid
            grid = np.zeros((grid_height, grid_width))
            
            # Place random obstacle within 4x8 meter area
            obstacle_x = random.choice(obstacle_area_x)
            obstacle_y = random.choice(obstacle_area_y)
            obstacle = (obstacle_x, obstacle_y)
            
            # Avoid placing obstacle at start or goal
            if obstacle == start_grid or obstacle == goal_grid:
                continue
            
            grid[obstacle_x, obstacle_y] = 1  # Single cell obstacle
            print(f"Obstacle placed at grid cell {obstacle}")
            ...
            # Visualization
            visualize_path(grid, path, start_grid, goal_grid, obstacle)
